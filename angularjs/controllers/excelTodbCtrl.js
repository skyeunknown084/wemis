HiMirrorSite.controller('excelTodbCtrl', function($scope,$rootScope,skinCareProdService) {

    $rootScope.skinproduct_mgt = localStorage.skinproduct_mgt;
    $rootScope.csv_converter = localStorage.csv_converter;
    $rootScope.EC_link_fetch = localStorage.EC_link_fetch;
    $rootScope.beautybox_mgt = localStorage.beautybox_mgt;
    $rootScope.maintenance_mgt = localStorage.maintenance_mgt;
    $rootScope.skinraw_mgt = localStorage.skinraw_mgt;
    $rootScope.device_mgt = localStorage.device_mgt;
    $rootScope.bodyraw_mgt = localStorage.bodyraw_mgt;
    $rootScope.skinreport_summary_mgt = localStorage.skinreport_summary_mgt;
    $rootScope.tips_mgt = localStorage.tips_mgt;
    $rootScope.member_mgt = localStorage.member_mgt;
    renewCacheTime();
	
	//alert('test');
	skinCareProdService.scpRawList()
		.then(function(data){
			//console.log(data);
			$scope.rawSCP = data.data;
			//$scope.rawSCPID = 
			//console.log($scope.rawSCP);
		});
	...
	$scope.saveSCP = function(id){
		//alert(id);
		$rootScope.loader = true;
		skinCareProdService.toSCP(id)
		.then(function (data){
			var rawtoscp = data.data;
			var toscpStat = rawtoscp.data;
			$rootScope.loader = false;
			alert(toscpStat.text);
			$scope.refresh();
	...
	$scope.toSCP = function(item) {
		...
		//****** Multiple and Select All for Delete 
		$("#checkAll").change(function () {
			$("input:checkbox").prop('checked', $(this).prop("checked"));
			if ($scope.selectAllRawSCP == true) {
				//var scapBatchItems = [];
				var scapBatchItemsSave = [];
				var scpItemArray = [];
				angular.forEach($scope.rawSCP, function(item) {
				item.checked = $scope.selectAllRawSCP;	
					if (item.checked) 
					//console.log(item);
					scpItemArray.push(item.id);
				});
			...
			/* var rakBatchItemsSave = rakBatchItems;
			var jsonToCsv = JSON.stringify(rakBatchItemsSave, undefined, 4); */
