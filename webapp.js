$(document).ready(function(){
  ...
  var table_companies = $('#table_companies').dataTable({
    ...
    "columns": [
      { "data": "ID" },
      { "data": "fname",   "sClass": "lname" },
      { "data": "age",    "sClass": "integer" },
      { "data": "email" },
      { "data": "address" },
  ...
  $(document).on('click', '#add_member', function(e){
    ...
    $('#form_company #id').val('');
    $('#form_company #fname').val('');
    $('#form_company #lname').val('');
    // $('#form_company #revenue').val('');
    $('#form_company #age').val('');
      ...
      request.done(function(output){
        if (output.result == 'success'){
          ...
          table_companies.api().ajax.reload(function(){
            ...
            var fname = $('#fname').val();
            show_message("Name '" + fname + "' added successfully.", 'success');
          }, true);
        } else {
          hide_loading_message();
  ...
  $(document).on('click', '.function_edit a', function(e){
    ...
    request.done(function(output){
      if (output.result == 'success'){
        ...
        $('#form_company #fname').val(output.data[0].fname);
        $('#form_company #lname').val(output.data[0].lname);
        $('#form_company #age').val(output.data[0].age);
      ...
      request.done(function(output){
        if (output.result == 'success'){
          ...
          table_companies.api().ajax.reload(function(){
            hide_loading_message();
            var fname = $('#fname').val();
            show_message("Company '" + fname + "' edited successfully.", 'success');
          }, true);
        } else {
          hide_loading_message();
    ...
    if (confirm("Are you sure you want to delete '" + fname + "'?")){
      ...
      request.done(function(output){
        if (output.result == 'success'){
          ...
          table_companies.api().ajax.reload(function(){
            hide_loading_message();
            show_message("Name '" + fname + "' deleted successfully.", 'success');
          }, true);
        } else {
          hide_loading_message();
